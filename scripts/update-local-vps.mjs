...
const vpsLocalPlugins = [
  { id: "shared-runtime", directory: "shared-runtime" },
...
export async function validateCandidateFetchInputs(candidateDir) {
  const rejectedConfigPaths = [
    ".npmrc",
    ".pnpmfile.cjs",
    path.join("local", "plugins", "dir-skills", ".npmrc"),
  ];
  ...
  if (
    ...
    throw new Error("dir-skills package lock must use lockfile version 3");
  }
  ...
  for (const [packagePath, entry] of Object.entries(lock.packages)) {
    ...
    if (
      ...
      throw new Error(
        `dir-skills package lock entry is unsafe: ${packagePath}`,
      );
    }
    ...
    if (
      ...
      throw new Error(`dir-skills package source is unsafe: ${entry.resolved}`);
    }
    tarballs.add(resolved.href);
  }
...
export function candidateCommandNeedsNetwork(command) {
  return (
    ...
    (command[0] === "npm" &&
      ((command[1] === "cache" && command[2] === "add") ||
        (command[1] === "install" && command.includes("--ignore-scripts"))))
  );
}
...
export function syncVpsPlugins({ cli, candidateDir }, execute = run) {
  const snapshots = vpsLocalPlugins.map((plugin) => ({
    ...plugin,
    source: readVpsPluginSource(cli, plugin.id, execute),
  }));
  ...
  try {
    for (const plugin of snapshots) {
      ...
      touched.add(plugin.id);
      for (const step of pluginSyncPlan(
        plugin.id,
        plugin.source?.requested ?? null,
        expected,
      )) {
        if (step.action === "install") {
          installVpsPluginSource(cli, { requested: expected }, execute);
        } else {
          execute(cli, ["plugin", step.action, step.id]);
        }
      }
    }
  } catch (error) {
...
function gitOutput(args, options = {}) {
  return run("git", args, { ...options, capture: true }).stdout;
}
...
async function createCandidate({
  ...
  const npmTarballs = await validateCandidateFetchInputs(releaseDir);
  const commands = candidateBuildCommands(dataDir, cacheDir, npmTarballs);
        ...
        },
        {
          ...
          runtime: {
            ...
            installPlugins: async () => {
              ...
              syncVpsPlugins({ cli, candidateDir });
            },
          },
        },
      );
