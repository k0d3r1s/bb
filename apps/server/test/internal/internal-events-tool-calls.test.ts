...
): ReturnType<typeof registerHostRpcResponder> {
...
describe("internal event and tool-call routes", () => {
  ...
  async function settleWorktreeAttach(harness: TestAppHarness) {
    const attachCommand = await waitForQueuedCommand(
      harness,
      ({ command }) =>
        command.type === "environment.attach" && command.path === WORKTREE_PATH,
    );
    if (attachCommand.command.type !== "environment.attach") {
      throw new Error("Expected environment.attach command");
    }
  ...
  it("creates a worktree from the exact checkout and queues continuation", async () => {
    await withTestHarness(async (harness) => {
      ...
      await settleWorktreeAttach(harness);
      const response = await responsePromise;
        ...
        contentItems: [
          {
            ...
            text: expect.stringMatching(
              /uncommitted changes[\s\S]*git stash push --include-untracked[\s\S]*bb_enter_worktree[\s\S]*git stash pop/,
            ),
          },
        ],
      });
