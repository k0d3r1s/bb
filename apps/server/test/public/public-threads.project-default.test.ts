...
describe("project-default thread environment", () => {
  it("resolves project-default to the project's shared checkout", async () => {
    await withTestHarness(async (harness) => {
      ...
      if (intent.type !== "reuse")
        throw new Error("Expected a reused checkout");
      expect(getEnvironment(harness.db, intent.environmentId)).toMatchObject({
        ...
        environmentProviderSelection: {
          ...
          inputs: { path: "/tmp/project-default-source" },
        },
      });
  ...
  it("resolves plain-text thread mentions without polluting the title fallback", async () => {
    await withTestHarness(async (harness) => {
      installDefaultEnvironmentProviders();
      const { host } = seedHostSession(harness.deps, {
        id: "host-thread-mention-create",
      });
      seedPrimaryHost(harness.deps, host.id);
      const { project } = seedProjectWithSource(harness.deps, {
        hostId: host.id,
        path: "/tmp/thread-mention-create",
      });
      const source = createThread(harness.db, noopNotifier, {
        projectId: project.id,
        providerId: "codex",
        ...
        segments: [
          {
            ...
            sourceKey: "1",
            sourceSeq: 1,
            text: "Recovered integration context.",
          },
        ],
      });
      const prompt = `Continue from @thread:${source.id}`;
      ...
      expect(getThread(harness.db, created.id)?.titleFallback).toBe(prompt);
      const storedInput = getThreadProvisionContext(harness.db, created.id)
        ?.request.input;
      expect(storedInput).toHaveLength(2);
      expect(storedInput?.[1]).toMatchObject({
        type: "text",
        text: expect.stringContaining("Recovered integration context."),
