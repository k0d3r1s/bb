...
describe("bb guide command output", () => {
  ...
  it("bb guide unknown chapter lists available chapters", async () => {
    ...
    expect(errorOutput).toContain(
      "Available: threads, environments, agent-configuration, providers, projects, machines, maintenance, terminals, browser, customization, plugins, automations, spend, json.",
    );
  });
  ...
  it("bb guide commands rejects a group that does not exist", async () => {
    ...
    expect(errorOutput).toContain(
      "Command groups: maintenance, browser, status",
    );
  }, 30_000);
